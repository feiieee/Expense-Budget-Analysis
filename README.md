...
本仓库是面向**财务分析 / 风控 / 运营数据**岗位的简历实战项目。以某企业 2026 年 1—8 月费用报销数据为样本，完成从**数据建模 → SQL 多维分析 → Power BI 可视化看板**的完整流程，重点练习预算执行监控、超支预警和趋势分析能力。
...
3. **查询二 · 预算达成率预警**：部门 × 费用类别的预算达成率（实际支出 ÷ 月度预算 × 8 × 100%），筛选超过 80% 的预警数据
...
![查询三：各部门 Top3 报销](查询三_各部门Top3报销.png.png)

![查询四：月度环比增长率](查询四_月度环比增长率.png.png)

![查询五：达成率排名](查询五_达成率排名.png.png)

## 分析结论

> 运行 SQL 后按实际结果填写，示例：

- 2026 年 1—8 月全公司「已通过」报销总额约 273,320 元，其中研发部最高（65,330 元）。
- 月度报销金额整体呈上升趋势，8 月环比增长 XX%（以实际查询结果为准）。
- 预算达成率按 80% 阈值暂无超支类别，可将预警阈值下调（如 30%）展示更敏感的风险预警。
...
├── 看板_总览.png.png                 # Power BI 看板截图
├── 查询三_各部门Top3报销.png.png      # 查询三结果
├── 查询四_月度环比增长率.png.png      # 查询四结果
└── 查询五_达成率排名.png.png          # 查询五结果
```
